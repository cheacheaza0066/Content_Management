  ...
  @foreach($news as $row)
      @if (++$i === $numItems)
        <div class="item-1">
          ...
          <a href="{{url('admin/detail/news/'.$row->id)}}"  class="card">
            <img src="{{asset($row->image)}}" class="image-main">

              <article>
                <h1 class="fw-bold" style="font-size: 50px">{{Str::limit($row->title,100)}}</h1>
                <p class="fw-normal" style="font-size: 20px">{{Str::limit($row->content,150)}}</p>
              </article>
          </a>
        </div>
      @endif
  ...
  @endforeach
    ...
    <div class="item">
      ...
      <a href="{{url('admin/detail/news/'.$row->id)}}" class="card">
        ...
        <article>
          <h3 class="fw-bold" style="font-size: 30px">{{Str::limit($row->title,85)}}</h3>
          <br><br>
          <p class="" style="font-size: 20px">{{Str::limit($row->content,200)}}</p>
      </article>
      </a>
    </div>
    @endforeach
